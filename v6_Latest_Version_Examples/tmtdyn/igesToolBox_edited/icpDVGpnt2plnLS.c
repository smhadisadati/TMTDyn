 ...
 *
 * Commpile in Matlab by running: mex -v icpDVGpnt2plnLS.c
 *
 * Usage:
 *
 * icpDVGpnt2plnLS(DATA,quality,quasirand,sizerand,randincreasing,maxiter,SURFDATA,DVGdata,R,t)
 *
 * Input:
 *
    ...
    for (ii=0;ii<ICP_iters;ii++){
        ...
        for (dataIndRndIndex=0;dataIndRndIndex<numRndInIters;dataIndRndIndex++){
            ...
            if (tmpWgh>0.0){
                ...
                doubleVar=(dataTemp[0]-MM[0])*modelSurfaceData[fourteencloIndTmp+11]+(dataTemp[1]-MM[1])*modelSurfaceData[fourteencloIndTmp+12]+(dataTemp[2]-MM[2])*modelSurfaceData[fourteencloIndTmp+13];
                
                doubleVars[5]=tmpWgh*doubleVar;
                doubleVars[6]=tmpWgh;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[]){
    ...
    icpDVGpnt2plnLS(mxGetPr(data), mxGetPr(quality), (unsigned int)mxGetN(data), (unsigned int*)mxGetPr(randvec), (unsigned int)mxGetScalar(sizerand), (unsigned int)mxGetScalar(randincrease), (unsigned int)mxGetScalar(maxiter), mxGetPr(surfdata), DVGdata, mxGetPr(rotationMatrixIO), mxGetPr(transVecIO));

                    ...
                    for i_dof_mult = 1 : dof_mult
                        ...
                        if isempty( joint(j_count).dof(nj_dof).equal2 )
							...
							if isnan( joint(j_count).dof(nj_dof).spring.init(n_mesh) )
								joint(j_count).dof(nj_dof).spring.init(n_mesh,i_dof_mult) = joint(j_count).dof(nj_dof).init(n_mesh) ;
							end
                            ...
                            nq_mat(3) = dof_m( 1 , 1 , i_dof_mult , temp(3) , temp(2) , temp(1) ) ; % only similar q
                        end
                        ...
                        switch joint(nq_mat(1)).dof(nq_mat(2)).dir(n_mesh)
...
%                         rotrans(i2) = q(end-dof_mult+1:end) * joint(j_count).dof(nj_dof).s.' ;
                        rotrans(i2) = q(nq_mat(3)-dof_mult+1:nq_mat(3)) * joint(nq_mat(1)).dof(nq_mat(2)).s.' ;

...
void int_mid_step(const emlrtStack *sp, real_T t, const real_T z[36], struct0_T *
                  b_par)
{
  real_T l;
  int32_T i2;
  ...
  if ((i2 < 1) || (i2 > 19)) {
    emlrtDynamicBoundsCheckR2012b(i2, 1, 19, &e_emlrtBCI, sp);
  }
  ...
  for (s = 0; s < i2; s++) {
    st.site = &m_emlrtRSI;
    sprdmpF19(&st, b_par->var, z, 1.0E-6 + (real_T)s * ds, unusedU0, unusedU1,
  ...
  if ((i2 < 1) || (i2 > 795)) {
    emlrtDynamicBoundsCheckR2012b(i2, 1, 795, &d_emlrtBCI, sp);
  }

  ds = b_par->var[4];
  for (s = 0; s < 6; s++) {
    b_par->var[21 + s] = b_par->p[(i2 + 795 * (1 + iv5[s])) - 1];
    b_par->var[s + 27] = b_par->f_ex[(i2 + 795 * (1 + s)) - 1];
  }

...
body(1).m = m_e ;
body(1).I = I_e ;
body(1).l_com = [ 0 0 0.2/2 ] ;

body(2).type = 'rigid' ;
body(2).tip = [ 0 0 0.2 ] ;
body(2).m = m_e ;
body(2).I = I_e ;
body(2).l_com = [ 0 0 0.2/2 ] ;
...
joint(1).dof(1).init = pi / 2 ;
joint(1).dof(1).spring = [ 0 , 0 ] ;
joint(1).dof(1).damp = [ 0.1 , 0 ] ;

% a  module as external load
joint(2).first = 1 ;
joint(2).second = 2 ;
joint(2).tr(1).trans = [ 0 0 0.2 ] ; % this is mid-base sensor position
joint(2).tr(1).rot = [ 2 inf ] ;
joint(2).tr2nd = [] ; % definitions
joint(2).dof(1).equal2 = [] ;
joint(2).dof(1).init = 0 ;
joint(2).dof(1).spring = [ 0 , 0 ] ;
joint(2).dof(1).damp = [ 0.1 , 0 ] ;
...
[ t , z , tfinal ] = Sim_test_v0_1( qf , uf , lambdaf , sf , qf0 , par ) ;
figure ; plot( t , z(:,1:2) ) ;

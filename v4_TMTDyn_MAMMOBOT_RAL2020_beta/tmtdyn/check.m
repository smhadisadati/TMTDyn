...
for i_j = 1 : numel( joint )
    ...
    [ n_mesh , ind ] = max( [ numel( joint(i_j).second(1,:) ) , numel( joint(i_j).first(1,:) ) ] ) ;
    ...
    if numel( joint(i_j).first(1,:) ) ~= numel( joint(i_j).second(1,:) )
        if ind == 2
            if numel( joint(i_j).second(1,:) ) == 1
                joint(i_j).second(1,2:n_mesh+1) = 1 ;
            else
                if numel( joint(i_j).second(1,:) ) == 2
                    joint(i_j).second(1,2:n_mesh+1) = joint(i_j).second(1,2) ;
                else
            ...
            if numel( joint(i_j).first(1,:) ) == 1
                joint(i_j).first(1,2:n_mesh+1) = 1 ;
            else
                if numel( joint(i_j).first(1,:) ) == 2
                    joint(i_j).first(1,2:n_mesh+1) = joint(i_j).first(1,2) ;
                else
                    error( 'numel( joint(i_j).first ) ~= numel( joint(i_j).second )' ) ;
                end
            end
        ...
        for i2 = 1 : numel( rotrans )
            if isinf( rotrans(i2) )
                ...
                if ~isfield( joint(i_j) , 'dof' ) || isempty( joint(i_j).dof ) || numel( joint(i_j).dof ) < nq
                    ...
                    joint(i_j).dof(nq).spring.equal2{1:n_r+1} = deal( vpa( zeros(n_mesh,7) ) ) ;
                    joint(i_j).dof(nq).spring.rom_offset = zeros(n_mesh,1) ;
                    ...
                    joint(i_j).dof(nq).fit_type = ones(n_mesh,1) .* joint(i_j).rom.fit_type ;
                else
                    if ~isfield( joint(i_j).dof(nq) ,'equal2' ) || isempty( joint(i_j).dof(nq).equal2 )
                        joint(i_j).dof(nq).equal2{1:n_r+1} = deal( vpa( zeros(n_mesh,7) ) ) ; % [j_count, n_mesh, nj_dof, i_dof_mult, val_or_l_rom] x i_dof_mult -or- [n_j, n_d, n_h, i_r, l] x n_r
                    else
                        for i_r = 1 : n_r + 1
                            ...
                            if joint(i_j).dof(nq).equal2{i_r}(n_mesh,1) == i_j && joint(i_j).dof(nq).equal2{i_r}(n_mesh,2) == n_mesh
                                error( 'joint(i_j).dof(nq).equal2{i_r}(n_mesh,1) = same joint is not allowed!' ) ;
                            end
                            if numel( joint(i_j).dof(nq).equal2{i_r}(1,:) ) == 2 % [n_j, n_h]
                                joint(i_j).dof(nq).equal2{i_r} = [ joint(i_j).dof(nq).equal2{i_r}(:,1) ones(n_mesh,1) joint(i_j).dof(nq).equal2{i_r}(:,2) nan*ones(n_mesh,1) zeros(n_mesh,3) ] ;
                            end
                            if numel( joint(i_j).dof(nq).equal2{i_r}(1,:) ) == 3 % [n_j, n_h, i_r]
                                joint(i_j).dof(nq).equal2{i_r} = [ joint(i_j).dof(nq).equal2{i_r}(:,1) ones(n_mesh,1) joint(i_j).dof(nq).equal2{i_r}(:,2:3) zeros(n_mesh,3) ] ;
                            end
                        end
                    end
                    if ~isfield( joint(i_j).dof(nq) ,'rom_offset' ) || isempty( joint(i_j).dof(nq).rom_offset )
                        joint(i_j).dof(nq).rom_offset = zeros(n_mesh,n_r+1) ;
                    else
                        joint(i_j).dof(nq).rom_offset = ones(n_mesh,1) .* joint(i_j).dof(nq).rom_offset ;
                    end
                    if ~isfield( joint(i_j).dof(nq) ,'fit_type' ) || isempty( joint(i_j).dof(nq).fit_type )
                        joint(i_j).dof(nq).fit_type = ones(n_mesh,1) .* joint(i_j).rom.fit_type ;
                    else
                        joint(i_j).dof(nq).fit_type = ones(n_mesh,1) .* joint(i_j).dof(nq).fit_type ;
                    end
                    ...
                    if ~isfield( joint(i_j).dof(nq) ,'spring' ) || isempty( joint(i_j).dof(nq).spring )
                        ...
                        joint(i_j).dof(nq).spring.init_cart = zeros(n_mesh,1) ;
                        joint(i_j).dof(nq).spring.compr = ones(n_mesh,n_r+1) ;
                        joint(i_j).dof(nq).spring.equal2{n_r+1} = deal( vpa( zeros(n_mesh,7) ) ) ;
                        joint(i_j).dof(nq).spring.rom_offset = zeros(n_mesh,1) ;
                        joint(i_j).dof(nq).spring.fit_type = ones(n_mesh,1) .* joint(i_j).rom.spring_fit_type  ;
                    else
                        ...
                        if ~isfield( joint(i_j).dof(nq).spring ,'rom_offset' ) || isempty( joint(i_j).dof(nq).spring.rom_offset )
                            joint(i_j).dof(nq).spring.rom_offset = zeros(n_mesh,n_r+1) ;
                        else
                            joint(i_j).dof(nq).spring.rom_offset = ones(n_mesh,1) .* joint(i_j).dof(nq).spring.rom_offset ;
                        end              
                        if ~isfield( joint(i_j).dof(nq).spring ,'fit_type' ) || isempty( joint(i_j).dof(nq).spring.fit_type )
                            joint(i_j).dof(nq).spring.fit_type = ones(n_mesh,1) .* joint(i_j).rom.spring_fit_type ;
                        else
                            joint(i_j).dof(nq).spring.fit_type = ones(n_mesh,1) .* joint(i_j).dof(nq).spring.fit_type ;
                        end
                        if ~isfield( joint(i_j).dof(nq).spring ,'equal2' )
							...
							for i_r = 1 : n_r + 1
								...
								% Here, same joint is not allowed: joint(i_j).dof(nq).spring.equal2{i_r}(n_mesh,1) = i_j
								if numel( joint(i_j).dof(nq).spring.equal2{i_r}(1,:) ) == 2 % [n_j, n_h]
									joint(i_j).dof(nq).spring.equal2{i_r} = [ joint(i_j).dof(nq).spring.equal2{i_r}(:,1) ones(n_mesh,1) joint(i_j).dof(nq).spring.equal2{i_r}(:,2) zeros(n_mesh,4) ] ;                           
								end
								if numel( joint(i_j).dof(nq).spring.equal2{i_r}(1,:) ) == 3 % [n_j, n_h, i_r]
									joint(i_j).dof(nq).spring.equal2{i_r} = [ joint(i_j).dof(nq).spring.equal2{i_r}(:,1) ones(n_mesh,1) joint(i_j).dof(nq).spring.equal2{i_r}(:,2:3) zeros(n_mesh,3) ] ;                   
								end
							end							
                        end
                    end
                ...
                if ~isempty( joint(i_j).rom.order )
                    if ~isfield( joint(i_j).dof(nq) , 'init_s' ) || isempty( joint(i_j).dof(nq).init_s )
                        if ~isempty( joint(i_j).rom.init_s ) && numel( joint(i_j).rom.init_s(1,:) ) == numel( joint(i_j).dof(nq).init(1,:) )
                            joint(i_j).dof(nq).init_s = ones(n_mesh,1) .* joint(i_j).rom.init_s ;
                        else
                    ...
                    if ~isfield( joint(i_j).dof(nq).spring , 'init_s' ) || isempty( joint(i_j).dof(nq).spring.init_s )
                        if ~isempty( joint(i_j).rom.init_s ) && numel( joint(i_j).rom.init_s(n_mesh,:) ) == numel( joint(i_j).dof(nq).spring.init(n_mesh,:) )
                            joint(i_j).dof(nq).spring.init_s = ones(n_mesh,1) .* joint(i_j).rom.init_s ;
                        else
                            if numel( joint(i_j).dof(nq).init_s(1,:) ) == numel( joint(i_j).dof(nq).spring.init(1,:) )
                                joint(i_j).dof(nq).spring.init_s = ones(n_mesh,1) .* joint(i_j).dof(nq).init_s ;
                            else
                                for i_d = 1 : n_mesh
                    ...
                    if ~isnan( joint(i_j).dof(nq).spring.init(n_mesh,1) ) && numel( joint(i_j).dof(nq).spring.init_s(1,:) ) ~= numel( joint(i_j).dof(nq).spring.init(1,:) )
    ...
    if ~isempty( joint(i_j).fixed )
        ...
        if ~isfield( joint(i_j) ,'refbody' ) || isempty( joint(i_j).refbody )
            joint(i_j).refbody = ones(n_mesh,1) * [ 0 1 0 ] ; % [body_no mesh_no location_lengh]
        else
...
for i_b = 1 : numel( body )
    ...
    for j_count = 1 : numel( joint )
        ...
        if joint(j_count).second(1,1) == i_b
            n_mesh = numel( joint(j_count).first(1,:) ) - 1 ;
...
for i_l = 1 : numel( exload )
    ...
    if ~isempty( exload(i_l).exbody )
        ...
        n_mesh = numel( exload(i_l).exbody(1,:) ) ;
        if n_mesh > 1
            n_mesh = n_mesh - 1 ;
        else
        ...
        if ~isfield( exload(i_l) ,'refbody' ) || isempty( exload(i_l).refbody )
            exload(i_l).refbody = ones(n_mesh,1) * [ 0 1 0 ] ; % wrt ref frame by default: [body_no mesh_no location_lengh]
        else

...
static void trfRcprclBarrier(double *paramFinal, unsigned char iters, double *paramConstrMin, double *linzGrad, double *linzHess, double *newtonStp, double *quasum, double *datam, double *modelm, double *Spx, double *constTermCnsFcn, double *doubleVars){
    ...
    doubleVars[6]=newtonStp[0]-paramConstrMin[0];
    doubleVars[7]=newtonStp[1]-paramConstrMin[1];
    doubleVars[8]=newtonStp[2]-paramConstrMin[2];
    
    /* Scale the paramteters if some of the angles are too large (>pi/3) */
    if (newtonStp[0]>1.0472){
        doubleVars[9]=(1.0472-paramConstrMin[0])/doubleVars[6];
    }
    else if (newtonStp[0]<-1.0472){
        doubleVars[9]=-(1.0472+paramConstrMin[0])/doubleVars[6];
    }
    else {
        doubleVars[9]=1.0;
    ...
    if (doubleVars[9]<0.999){
        ...
        doubleVars[3]=paramConstrMin[3]+doubleVars[9]*(newtonStp[3]-paramConstrMin[3]);
        doubleVars[4]=paramConstrMin[4]+doubleVars[9]*(newtonStp[4]-paramConstrMin[4]);
        doubleVars[5]=paramConstrMin[5]+doubleVars[9]*(newtonStp[5]-paramConstrMin[5]);
    }
    else {
        doubleVars[0]=newtonStp[0];
        doubleVars[1]=newtonStp[1];
        doubleVars[2]=newtonStp[2];
        doubleVars[3]=newtonStp[3];
        doubleVars[4]=newtonStp[4];
        doubleVars[5]=newtonStp[5];
    }

...
[ world , body , joint , exload , mesh , par ] = template_builder() ;
...
par.opv = true ; % optimize generated files
par.simplify = true ; % optimize derivations
...
par.dyn = 1 ; % 1: dyn. sim. with Matlab func.s, 2: with C-mex func.s, 3: to used latest genarated C-Mex file
par.post_process = 1 ; % post-process using user code
...
joint(1).second = 1 ;
joint(1).tr.rot_type = 'angle_axis' ;
joint(1).tr.rot = [ inf 0 1 0 ] ;
...
mesh.tr(1).rot_type = 'angle_axis' ;
mesh.tr(1).rot = [ phi_sym 0 1 0 ] ;
...
mesh.joint(1).tr.rot_type = 'non_unit_quat' ;
mesh.joint(1).tr.rot = [ inf inf inf inf ] ;
mesh.joint(1).dof(4).init = 1 ;

% fabric EBA
mesh.joint(2).spring.coeff = [ diag( K_v ).' diag( K_u ).' ] ;
% mesh.joint(2).spring.init = [ 0 0 nan 0 0 0 ] ;
mesh.joint(2).spring.init = nan ;
...
joint(2).fixed = ones(1,3) ;

joint(3).first = 1 ;
joint(3).second = 14 ;
joint(3).tr.trans = [ -l_x1 0 -l_z1 ] ;
joint(3).fixed = ones(1,3) ;
...
par.t_init = 0;
par.t_final = 1;
results = pipeline (par, world, body, joint, exload, mesh);

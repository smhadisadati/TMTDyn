...
for i = 1 : ss
    [ ~ , par_mex ] = int_mid_step( t(i) , z(i,:) , par.par_mex ) ;
    vars = par_mex.var ;
    temp2 = rksF_mex( vars , z(i,:) , 0 ) ;
    for i_sd = 1 : par.n_ks_anim
        r_anim.sprdmp(i_sd).r(1:2,1:3,i) = [ temp2(i_sd,1:3) ;
                                             temp2(i_sd,4:6) ] ;
        if par.anim_frame
    ...
    for i_s = 1 : par.n_animpoints + 1
        ...
        for i_m = 1 : par.n_mass_anim
            if ~isnan( temp1(i_m,4) ) % rigid
                r_anim.mass(i_m).r(i_s,1:3,i) = ( temp1(i_m,4:6) - temp1(i_m,1:3) ) * dl + temp1(i_m,1:3) ;
                if par.anim_frame
...
for n_t = 1 : sk : ss % update the points
    ...
    for i = 1 : par.n_ks_anim % creat lines ending to each joint
        plot3( [ 0 r_anim.sprdmp(i).r(:,4,n_t) ] , [ 0 r_anim.sprdmp(i).r(:,5,n_t) ] , [ 0 r_anim.sprdmp(i).r(:,6,n_t) ] , 'LineWidth' , 2 ,'Color' , clr{mod(i,6)+1} );
        hold on
    end

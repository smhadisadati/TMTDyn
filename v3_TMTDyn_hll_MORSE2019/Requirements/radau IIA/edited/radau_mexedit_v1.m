...
function [tout, yout] = radau_mexedit_v1(tspan,y0,vars)
%
...
% Default options values
Op.AbsTol            = 1e-6 * ones(numel(y0),1);       % General parameters
Op.RelTol            = 1e-3 * ones(numel(y0),1);
Op.InitialStep       = 1e-2;
...
[tout,yout] = radausolver(tspan,y0,Op,vars);
...
function [tout, yout] = radausolver(tspan,y,Op,vars)
...
InitNbrStg = true;
[T, TI, ValP, C, Dd] = Coertv(MinNbrStg,MaxNbrStg,RealYN,InitNbrStg);
InitNbrStg = false;
[T, TI, ValP, C, Dd] = Coertv(NbrStg,NbrStg,RealYN,InitNbrStg);
...
% ------- COMPUTE THE MATRICES E1 AND E2 AND THEIR DECOMPOSITIONS L U P 
  if NeedNewLU                                           
    [L, U, P, U_Sing] = DecomRC(h,ValP,Mass,Jac,RealYN);
...
%       end
      f(:,q) = EOM(t+C(q)*h, y+real(z(:,q)),vars); % OdeFcn needs parameters
      if any(isnan(f))
...
% ------- SOLVE THE LINEAR SYSTEMS    % Ligne 1037                    
    z                 = Solvrad(z,w,ValP,h,Mass,RealYN,U,L,P); 
...
%                           First, Reject,Stat,OdeFcn,t,y,varargin{:});
  [err,Stat] = Estrad(z,Dd,h,Mass,Scal,f0, ...
                          First, Reject,Stat,t,y,vars,U,L,P);
...
%     if nargout > 0
      switch OutFlag        
        ...
        case 2          % Computed points, with refinement 
          ...
          ii           = oldnout+1:nout-1;
          tinterp      = t+h*S - h;
          yinterp      = ntrprad(tinterp,t,y,h,cont,C);
          tout(ii)     = tinterp;
        ...
        case 3          % Output only at tspan points
          ...
          while next <= ntspan
            ...
            yinterp      = ntrprad(tspan(next),t,y,h,cont,C);
...
function yinterp = ntrprad(tinterp,t,y,h,cont,C)
% global C
...
function  [L, U, P, U_Sing] = DecomRC(h,ValP,Mass,Jac,RealYN)
% global L U P 
...
function z = Solvrad(z,w,ValP,h,Mass,RealYN,U,L,P)
...
if ~isempty(tout)
  yinterp = ntrprad(tout,t,y,h,cont,C);
...
function [T, TI, ValP, C, Dd] = Coertv(MinNbrStg,MaxNbrStg,RealYN,Init)
...
T = 0 ; TI = 0 ; ValP = 0 ; C = 0 ; Dd = 0 ;

if Init
  for NbrStg = MinNbrStg:MaxNbrStg
    switch NbrStg
      case 1
        [T_1, TI_1, C1, ValP1, Dd1] = Coertv1(RealYN);
      case 3
        [T_3, TI_3, C3, ValP3, Dd3] = Coertv3(RealYN);
      case 5
        [T_5, TI_5, C5, ValP5, Dd5] = Coertv5(RealYN);
      case 7
        [T_7, TI_7, C7, ValP7, Dd7] = Coertv7(RealYN);
    end
  end
else
...
function [T_1, TI_1, C1, ValP1, Dd1] = Coertv1(RealYN)
...
function [T_3, TI_3, C3, ValP3, Dd3] = Coertv3(RealYN)
...
function [T_7, TI_7, C7, ValP7, Dd7] = Coertv7(RealYN)
% See page 78 in Stiff differential Equations:  Radau IIa order 5
% Butcher representation
%  C(1) |  A(1,1)  A(1,2)  A(1,3)
%  C(1) |  A(2,1)  A(2,2)  A(2,3)

...
if par.plotIC; fig_rom = figure; else; fig_rom = []; end
for j_count = 1 : n_j % look into all joints
    ...
    for n_mesh = 1 : joint(j_count).n_mesh
        ...
        rotrans = emps; rotrans_spring = emps;
        for i3 = 1 : nj_tr
            rotrans = [ rotrans; sym( [ joint(j_count).tr(i3).trans , joint(j_count).tr(i3).rot , i3 ] ) ] ;
            rotrans_spring = [rotrans_spring; sym( [ joint(j_count).tr(i3).trans , joint(j_count).tr(i3).rot , i3 ] ) ] ;
            
            isq = 0 ; % if quat. rot. contains states
            for i2 = 1 : numel( rotrans(end,:) ) - 1
                if rotrans(end,i2) == inf
                    ...
                    if ~isempty( joint(j_count).rom.order ) % reduced order model
                        ...
                        % s offset:
                        if joint(j_count).dof(nj_dof).spring.rom_offset(n_mesh) == 0
                        ...
                        % rotrans for Q & TQ
                        rotrans(end,i2) = smplfy( joint(j_count).dof(nj_dof).rom_series{n_mesh}(1,end) ) ;
                        rotrans_spring(end,i2) = smplfy( joint(j_count).dof(nj_dof).spring.init_rom_series{n_mesh}(end) ) ;
                    else
                        rotrans(end,i2) = q(nq);
                        rotrans_spring(end,i2) = q(nq);
                    end
                end
            end
            ...
            if i3 > 1 && ... % combine similar single-axis rotations for derivation efficiency (no need for transfomrations)
                    ...
                    min( double( rotrans(end,1:3) == 0 ) ) && ... % pure rotation (no translation)
                    min( double( rotrans(end,5:7) == rotrans(end-1,5:7) ) ) % similar axis
                rotrans(end-1,4) = rotrans(end-1,4) + rotrans(end,4) ; % add-up angles
                rotrans(end,:) = [] ;
                rotrans_spring(end-1,4) = rotrans_spring(end-1,4) + rotrans_spring(end,4) ;
                rotrans_spring(end,:) = [] ;
            end
        end
        
        for i3 = 1 : numel( rotrans(:,1) ) % derive local transformations for each joint
            [ Q_temp , TQ_temp ] = TQ_mat( rotrans(i3,:) , [ q s ] , joint(j_count).tr(rotrans(i3,8)).rot_type , joint(j_count).tr(rotrans(i3,8)).init_quat );
            ...
            if ~isempty( joint(j_count).rom.order ) % reduced order model
                [ Q_temp , TQ_temp ] = TQ_mat( rotrans_spring(i3,:) , [ q s ] , joint(j_count).tr(rotrans_spring(i3,8)).rot_type , joint(j_count).tr(rotrans(i3,8)).init_quat );
...
if par.plotIC % compare overall fit curves with init data
    figure
    for j_count = 1 : n_j
        for n_mesh = 1 : joint(j_count).n_mesh
    ...
    for j_count = 1 : n_j % look into all joints
        ...
        if joint(j_count).second(1,1) == b_count % if the joint connects this body to another one,
            ...
            for n_mesh = 1 : body(b_count).n_mesh
                ...
                mass(n_m).Dd(4:6,:) = smplfy( myJacobian( body(b_count).omega(n_mesh).abs_loc.' , q ) ) ;
                ...
                mass(n_m).Dd(1:3,:) = smplfy( myJacobian ( body(b_count).v_com(n_mesh,:).' , q ) ) ;
...
switch rot_type
    ...
    case 'frenet_serret'
        ...
        Q(2,1) = ( R(3,2) - R(2,3) ) / ( 4 * Q(1) ) ;
        Q(3,1) = ( R(1,3) - R(3,1) ) / ( 4 * Q(1) ) ;
        Q(4,1) = ( R(2,1) - R(1,2) ) / ( 4 * Q(1) ) ;
...
if isnan( joint(j_count).dof(nj_dof).equal2{1}(n_mesh,4) ) % init from other dof
    ...
    if numel( joint(j_count).dof(nj_dof).fit_type(n_mesh,:) ) > 1 && joint(j_count).dof(nj_dof).fit_type(n_mesh,1) >= 2 % spline init & arg cond.s
        ...
        if  joint(j_count).dof(nj_dof).spring.fit_type(n_mesh,1) == 3 % spline_logistic_function
            lfk = 200; % logistic growth rate (curve steepness)
            if n_kn > 1
...
if isnan( joint(j_count).dof(nj_dof).spring.init(n_mesh,1) ) % spring.init from dof.init: uses S, b, q_equal from dof.init
    ...
    % needed only once for each mesh element
...
if par.plotIC % compare fit curve with init data
    figure(fig_rom)

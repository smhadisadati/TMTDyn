...
// void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  FILE *yout_file_current = fopen("results_current.txt", "w");
  if (yout_file == NULL || yout_file_current == NULL) {
  ...
  while(1) {
    ...
    if (retval == CV_SUCCESS) {
      ...
      fprintf(yout_file_current, "%f", t); // file print data
      for (int i = 0; i < NEQ; i++) {
        ...
        fprintf(yout_file_current, "\t%f", Ith(y,i)); // file print data
      }
      //printf("\n");
      fprintf(yout_file, "\n");
      fseek(yout_file_current, 0, SEEK_SET); // move to the begining of the current data file
      // iout++;
      tout += tspan[1];
    }

...
body(1).curve.dof(1).init = l_1 ; % flexible body states' initial condition
...
body(2).m = m_2 ;
body(2).I = I_2 ;
body(2).l_com = [ 0 0 l_2/2 ] ;

body(3).type = 'EB-CC' ; % 'rigid', 'EB-CC', 'VC', default: rigid
body(3).m = m_3 / l_3 ; % mass or mass/length
body(3).I = I_3 / l_3 ; % inertia around COM expressed in local frame
body(3).l_com = [ 0 0 l_3/2 ] ; % COM position vector w.r.t & expressed in local frame
body(3).tip = l_3 ; % tip position vector w.r.t & expressed in local frame
body(3).curve.stiff = [ kt3 0 0 0 0 kbr3 ] ; % curvilinear translational and rotational stiffness: [ t n b tr nr br ]                      % set to Ea/l, EI/l or GJ/l from EB relation
body(3).curve.viscdamp = [ cvt3 0 0 0 0 cvbr3 ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
body(3).curve.couldamp = [ cct3 0 0 0 0 ccbr3 ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
body(3).curve.par = [ l_3 inf 0 ] ; % put 'inf' for curve states to be dof and values for fixed directions
body(3).curve.dof(1).equal2 = [] ; % equal to another curve parameter [ bodyNumber dofNumberInBody ]
body(3).curve.dof(1).init = eps ; % flexible body states' initial condition
body(3).curve.dof(1).spring = [ 0 0 ] ;
body(3).curve.dof(1).damp = [ 0 0 ] ;
...
joint(3).first = 2 ;
joint(3).second = 3 ;
joint(3).tr(1).trans = [ 0 0 l_2 ] ; % this is mid-base sensor position
joint(3).tr(1).rot = [ 0 0 ] ;

exload = [] ; % definition
exload(1).body = 3 ; % point of contact body number

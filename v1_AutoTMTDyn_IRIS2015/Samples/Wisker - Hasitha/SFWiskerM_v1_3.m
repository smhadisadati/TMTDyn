...
body(1).m = m_3 / l_3 ; % mass or mass/length
body(1).I = I_3 / l_3 ; % inertia around COM expressed in local frame
body(1).l_com = [ 0 0 l_3/2 ] ; % COM position vector w.r.t & expressed in local frame
body(1).tip = l_3 ; % tip position vector w.r.t & expressed in local frame
body(1).curve.stiff = [ kt3 0 0 0 0 kbr3 ] ; % curvilinear translational and rotational stiffness: [ t n b tr nr br ]                      % set to Ea/l, EI/l or GJ/l from EB relation
body(1).curve.viscdamp = [ cvt3 0 0 0 0 cvbr3 ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
body(1).curve.couldamp = [ cct3 0 0 0 0 ccbr3 ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
body(1).curve.par = [ l_3 inf 0 ] ; % put 'inf' for curve states to be dof and values for fixed directions
body(1).curve.dof(1).equal2 = [] ; % equal to another curve parameter [ bodyNumber dofNumberInBody ]
body(1).curve.dof(1).init = eps ; % flexible body states' initial condition
...
joint(1).tr(1).rot = [ 2 inf ] ; % position vector w.r.t. & exp. in prev. joint local frame
joint(1).tr2nd = [] ; % definitions
joint(1).dof(1).equal2 = [] ;
joint(1).dof(1).init = 0 ;
joint(1).dof(1).spring = [ kbr1 , 0 ] ;
joint(1).dof(1).damp = [ cvbr1 , ccbr1 ] ;
...
par.t0 = 0 ; par.dt = 1e-3 ; % final tmie: 218
par.stepT = 1e-5 ; % time step for sensor readings is 0.025

...
body(1).CC_EB.l_com = [ l_b/2 0 0 ] ; % COM position vector w.r.t & expressed in local frame
...
body(1).CC_EB.par = [ inf inf 0 ] ; % put 'inf' for CC_EB states to be dof and values for fixed directions
...
[ t , z , tfinal ] = SimEoM_EB_v0_4_2D( qf , uf , lambdaf , sf , qf0 , par ) ;
% plot
...
title('curvature states') ; legend( '\theta_1' ) ; ylabel( '\theta [rad]' ) ; xlabel( 't [s]' ) ;
pause;

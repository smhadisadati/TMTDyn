        ...
        if joint(i_joint).second(1,1) == b_count % if the joint connects this body to another one,
            ...
            for i_copies = 1 : body(b_count).n_copy
                ...
                if par.derive == 2 % EOM simplification
                    mass(n_m).M(4:6,4:6) = sym( zeros(3,3) ) ; % slower absolute rotations compared to translations
                else
                ...
                if par.derive == 2 % EOM simplification
                    mass(n_m).T(4:6,:) = sym( zeros( 3 , numel( q ) ) ) ; % slower absolute rotations compared to translations
                    mass(n_m).Dd(4:6,:) = sym( zeros( 3 , numel( q ) ) ) ; % slow systems
                else
                ...
                if par.derive == 2 % EOM simplification
                    mass(n_m).Dd(1:3,:) = sym( zeros( 3 , numel( q ) ) ) ; % slow systems
                else

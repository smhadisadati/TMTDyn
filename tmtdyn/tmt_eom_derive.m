    ...
    for i_joint = 1 : n_jonts % look into all joints
        ...
        if joint(i_joint).second(1,1) == b_count % if the joint connects this body to another one,
            ...
            for i_copies = 1 : body(b_count).n_copy
                ...
                if par.derive == 2 % EOM simplification
                    mass(n_m).T(4:6,:) = zeros( 3 , size( q ) ) ; % slower absolute rotations compared to translations
                    mass(n_m).Dd(4:6,:) = zeros( 3 , size( q ) ) ; % slow systems
                else
                ...
                if par.derive == 2 % EOM simplification
                    mass(n_m).Dd(1:3,:) = zeros( size( mass(n_m).T(1:3,:) ) ) ; % slow systems
                else

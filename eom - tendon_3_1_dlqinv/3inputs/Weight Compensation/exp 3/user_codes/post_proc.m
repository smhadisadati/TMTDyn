...
% t_u = t(3:end) ;
% u_kappa = diff( z(:,end/2-[1,0]) , 2 ) ./ ( t(3:end) - t(2:end-1) ).^2 ; % last two inputs are relaxed-state curvetures
% u_slide = diff( z(:,end/2-2) , 2 ) ./ ( t(3:end) - t(2:end-1) ).^2 ; % input force
t_u = t(2:end) ;
u_kappa = smoothV( diff( z(:,end-[1,0]) ) ./ ( t(2:end) - t(1:end-1) ) , 200 ) ; % last two inputs are relaxed-state curvetures
u_slide = smoothV( diff( z(:,end-2) ) ./ ( t(2:end) - t(1:end-1) ) , 200 ) ; % input force for sliding
...
subplot(2,4,5); plot(t_u, u_kappa, '-' , 'LineWidth', 2); % last two inputs are relaxed-state curvetures
...
subplot(2,4,[2,6]); plot(t, z(:,end/2-3), '-' , 'LineWidth', 2); % last state is the sliding state (introducer feed)

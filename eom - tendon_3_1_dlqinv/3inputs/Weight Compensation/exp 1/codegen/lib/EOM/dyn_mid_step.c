...
{
  double varargin_2[6840];
  double varargin_1[1710];
  double dv[3];
  double dv1[3];
  double b_y1;
  double f_t_idx_0;
  ...
  /*  PD controller */
  /*  introducer load */
  memcpy(&varargin_1[0], &par_mex->user_pars.tip_load[0], 1710U * sizeof(double));
  for (offset = 0; offset < 4; offset++) {
    memcpy(&varargin_2[offset * 1710], &par_mex->user_pars.tip_load[offset *
  ...
  if (par_mex->user_pars.tip_load[1] < par_mex->user_pars.tip_load[0]) {
    for (offset = 0; offset < 855; offset++) {
      ...
      varargin_1[offset] = varargin_1[1709 - offset];
      varargin_1[1709 - offset] = xtmp;
    }

    for (mid_i = 0; mid_i < 4; mid_i++) {
      offset = mid_i * 1710 + 1710;
      for (k = 0; k < 855; k++) {
        low_ip1 = (offset + k) - 1710;
  ...
  f_t_idx_2 = 0.0;
  xtmp = 0.0;
  if ((t <= varargin_1[1709]) && (t >= varargin_1[0])) {
    offset = 1;
    low_ip1 = 2;
    high_i = 1710;
    ...
    if (xtmp == 0.0) {
      ...
      f_t_idx_1 = varargin_2[offset + 1709];
      f_t_idx_2 = varargin_2[offset + 3419];
      xtmp = varargin_2[offset + 5129];
    } else if (xtmp == 1.0) {
      f_t_idx_0 = varargin_2[offset];
      f_t_idx_1 = varargin_2[offset + 1710];
      f_t_idx_2 = varargin_2[offset + 3420];
      xtmp = varargin_2[offset + 5130];
    } else {
      ...
      b_y1 = varargin_2[offset + 1709];
      y2 = varargin_2[offset + 1710];
      if (b_y1 == y2) {
        f_t_idx_1 = b_y1;
      } else {
      ...
      b_y1 = varargin_2[offset + 3419];
      y2 = varargin_2[offset + 3420];
      if (b_y1 == y2) {
        f_t_idx_2 = b_y1;
      } else {
      ...
      b_y1 = varargin_2[offset + 5129];
      y2 = varargin_2[offset + 5130];
      if (b_y1 == y2) {
        xtmp = b_y1;
      } else {

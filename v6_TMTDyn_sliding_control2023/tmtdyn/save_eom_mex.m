...
if ismember( par.equil , [ 1 2 6 ] ) || ismember( par.dyn , [ 1 2 6 ] ) % not using old m-file or mex-file
    switch analysis_case
        ...
        case 'dyn' % Sundials ODE solver
            switch par.solver
                case { 1, 2, 4 , 5 , 6 , 7 } % matlab, sundials, radau, external, load_result
                    string_1 = [
    ...
    switch analysis_case
        case 'equil' % equil. (static) sym.
            string_1 = [ string_1 newline ...
                ...
                'z = [ z 0*z ]'' ;' newline ...
                newline ...
                'u = z( ' num2str( par.nq + par.nlambda + par.ngamma + par.nlambda + 1 ) ' : end ) ;' newline ...
...
%             end
            string_1 = [ string_1 newline ...
                ...'t ' newline ...
                newline ...
                'u = z( ' num2str( par.nq ) ' + ' num2str( par.nlambda ) ' + ' num2str( par.ngamma ) ' + ' num2str( par.nlambda ) ' + 1 : end ) ;' newline ...
                'uq = u( 1 : ' num2str( par.nq ) ' ) ;' newline ...
                newline ...
    ...
    if par.sparse
        ...
        string_1 = [ string_1 newline ...
            ...
            'Tc = [] ; Tct = Tj_u ; dc = [] ; in_c = [] ; kl_slide = [] ;' newline ] ;
    end
    ...
    for i = par.nq + 1 : par.n_sd
        if par.derive_collect == 1
            [ ~ , ~ , ~ , ~ , Ttu_temp ] = eval( [ 'sprdmpF' num2str(i) '( par_mex.var , zeros( 2 * ( par.nq + par.nlambda + par.ngamma + par.nlambda ) , 1 ) , 0 , 0 , 0 )' ] ) ;                    
        elseif par.derive_collect == 2
            Ttu_temp = eval( [ 'sprdmpTtu' num2str(i) '( par_mex.var , zeros( 2 * ( par.nq + par.nlambda + par.ngamma + par.nlambda ) , 1 ) , 0 , 0 , 0 )' ] ) ;
        end
        % Ttu_temp = [];
    ...
    switch analysis_case
        ...
        otherwise % dyn. sim.
            for i = 1 : par.n_cn
                if par.derive_collect == 1
                    string_all = [ string_all newline ...
                        '[ tc , tct , Dc , in , rc , uc , gains ] = cnstF' num2str(i) strmex '( par_mex.var , z , 0 , t ) ;' ] ;
                elseif par.derive_collect == 2
                    string_all = [ string_all newline ...
                        ...
                        'in = cnstIn' num2str(i) strmex '( par_mex.var , z , 0 , t ) ;' newline ...
                        'rc = cnstR' num2str(i) strmex '( par_mex.var , z , 0 , t ) ;'  newline ...
                        'uc = cnstU' num2str(i) strmex '( par_mex.var , z , 0 , t ) ;'  newline ...
                        'gains = cnstGains' num2str(i) strmex '( par_mex.var , z , 0 , t ) ;' ] ; 
                end
                if par.sparse
                    string_all = [string_all newline ...
                ...
                string_all = [ string_all newline ...
                    ...
                    'Tct = [ Tct , tct ] ;' newline ...
                    'dc = [ dc ; ( - Dc * uq ) ] ;' newline ...
                    'in_c = [ in_c ; in + gains(1) * rc + gains(2) * uc ] ; %% "in" is the constraint desired term ac_d+k1*uc_d+k0*rc_d' newline ...
                    'kl_slide = [ kl_slide ; gains(3:4) ] ;' newline ] ;
            end
            
    end
    
    str_Rayleigh_M_type = '' ; % 'constant' Rayleigh_M_type with constant str_Rayleigh_M_coeff
    if par.Rayleigh_M_type == 1 % str_Rayleigh_M_type with 'adaptive_svd' str_Rayleigh_M_coeff, i.e. based on SVD analysis
        str_Rayleigh_M_type = ' ./ svd( TMT ) ' ;
    end
    ...
    switch analysis_case
        case 'equil' % equil. (static) sym.
            if par.nlambda + par.ngamma == 0
                ...
                lg_str = [ ' + Tct * z(end/2-' num2str( par.nlambda + par.ngamma + par.nlambda ) '+1:end/2) ' ] ;                
            end
        ...
        otherwise % dyn. sim.
            switch par.control_type
                case 0 % jacobian_inverse
                    string_all = [ string_all newline ...
                        '%% EOM via jacobian inversion' newline ...
                        'TMTb = [ TMT , -Tct ; ' newline ... % concatanate with the desired trajectory jacobian
                        ...
                        'd_eom = [ ( TMfd + fj_k + fj_vd + fj_in + w_sd + w_in + w_vd_j + w_f - ( ' num2str( par.Rayleigh_M_coeff ) str_Rayleigh_M_type ' ) .* ( TMT * u(1:end-' num2str(par.nlambda+par.ngamma+par.nlambda) ') ) ) ; dc ] ;' newline ... % Rayleigh damping on the states only
                        newline ...
                        'dY = u ;' ] ;
                ...
                case 1 % input_output_feedback_linearization
                    string_all = [ string_all newline ...
                        '%% EOM via input_output_feedback_linearization' newline ...
                        'e = z( ( par.nq + par.nlambda + par.ngamma + 1 ) : ( end/2 )  ) ;' newline ...
                        'ddY_d = pinv( Tc ) * ( dc + e ) ;' newline ...
                        'd_eom_q = ( TMfd + fj_k + fj_in + w_sd + w_in + w_f ) ;' newline ...
                        newline ...
                        'U = pinv( Tct ) * ( TMT * ddY_d - d_eom_q ) ;' newline ...
                        newline ...
                        'de = in_c ;' newline ...
                        'dY = u ;' ] ;                    
                case 2 % weight_compensation_&_attractor_field
                    string_all = [ string_all newline ...
                        '%% weight_compensation_&_attractor_field' newline ...
                        'F_d = Tc'' * in_c ;' newline ...
                        'd_eom_q = ( TMfd + fj_k + fj_in + w_sd + w_in + w_f ) ;' newline ...
                        newline ...
                        'U = pinv( Tct ) * ( F_d - d_eom_q ) ;' newline ...
                        newline ...
                        'dY = u ;' ] ;
                case 3 % sliding_mode
                    string_all = [ string_all newline ...
                        '%% EOM via input_output_feedback_linearization' newline ...
                        'e = z( ( par.nq + par.nlambda + par.ngamma + 1 ) : ( end/2 )  ) ;' newline ...
                        'ddY_d = pinv( Tc ) * ( dc + in_c - diag( kl_slide(:,1) ) * e - diag( kl_slide(:,2) ) * sign( e ) ) ;' newline ...
                        'd_eom_q = ( TMfd + fj_k + fj_in + w_sd + w_in + w_f ) ;' newline ...
                        newline ...
                        'U = pinv( Tct ) * ( TMT * ddY_d - d_eom_q ) ;' newline ...
                        newline ...
    ...
    switch analysis_case
        case 'dyn'
            switch par.solver 
                case { 1 , 2 , 4 , 5 , 6 , 7 } % matlab, sundials_ode, radau, external, load_result
                    switch par.control_type
                        case 0 % jacobian_inverse
                            string_all = [ string_all newline ...
                                ...'ddY = TMTb \\ d_eom ;' newline newline ...
                                ...'dY = TMTu \\ u ;' newline ...
                                ...'ddY = pinv( TMTb ) * ( d_eom - ' num2str( par.Rayleigh_M_coeff ) ' * TMTb * dY ) ;' newline ... % Rayleigh damping on all states, inputs, lagrangian multipliers, etc
                                'ddY = pinv( TMTb ) * d_eom ;' newline ... % no Rayleigh damping or dmping on states only (all cases)
                                ...'dY = pinv( TMTu ) * u ;' newline ...
                                ...'ddY = lsqminnorm( TMTb , d_eom ) ;' newline ...
                                ...'dY = lsqminnorm( TMTu , u ) ;' newline ...
                                newline ...
                                'dz = [ dY ; ddY ];' ] ;                            
                        case { 1 , 2 , 3 } % input_output_feedback_linearization & weight_compensation_&_attractor_field & sliding_mode
                            string_all = [ string_all newline ...
                                'ddY = pinv( TMT , 1e-24 ) * ( d_eom_q + w_vd_j + fj_vd + Tct * U - ( ' num2str( par.Rayleigh_M_coeff ) str_Rayleigh_M_type ' ) .* ( TMT * u( 1 : ' num2str( par.nq ) ' + ' num2str( par.nlambda ) ' ) ) ) ;' newline ...
                                newline ...
                                'dz = [ dY ; ddY ; de ; U ];' ] ;                           
                    end                    
                case 3 % sundials_dae
                    string_all = [ string_all newline ...
                        newline ...
                        'res = [ ( TMTu * dz( 1 : ' num2str( par.nq ) ' + ' num2str( par.nlambda ) ' ) - u ) ; ' newline ... % TODO
...
if ismember( par.equil , [ 2 , 4 ] ) % derive EOM mex function
    ...
    switch analysis_case
        case 'equil' % equil. (static) sym.
            % mex
            vars_mex = { zeros( 1 , ( par.nq + par.nlambda + par.ngamma + par.nlambda ) ) } ;
...
if ismember( par.dyn , [ 2 , 4 ] ) % derive EOM mex function
    ...
    switch analysis_case
        case 'dyn' % dyn sim
            ...
            switch par.solver
                case { 1 , 2 , 4 , 5 , 6 , 7 } % matlab, sundials_ode, radau, external, load_result
                    vars_mex = { 0 , zeros( 2 * ( par.nq + par.nlambda + par.ngamma + par.nlambda) , 1 ) , par_mex } ;
                case 3 % sundials_dae
                    vars_mex = { 0 , zeros( 2 * ( par.nq + par.nlambda + par.ngamma + par.nlambda ) , 1 ) , zeros( 2 * ( par.nq + par.nlambda + par.ngamma + par.nlambda )  , 1 ) , par_mex } ;
            end

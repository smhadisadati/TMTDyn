...
l_b = 44e-3 ; l_fs = 17e-3 ; l_base = 17e-3 ;
l_e = l_b +  l_fs ;
...
n_seg = 3 ; % number of segments in the discrete VC

tspringCoeff = n_seg / ( lambdal ^ 2 * l_b / E / a_S0 ) ; % translational
tspringCoeff = 1 * tspringCoeff ; % corrected
tspringInit = 0;
tdampViscous = 1e-3;
tdampCoulomb = 0;
rspringCoeff = n_seg / ( lambdal ^ 3 * l_b / E / I_1 ) ; % rotational
...
for i = 1 : n_seg
    ...
    body(i).CC_EB.l_com = [ l_b/2/n_seg 0 0 ] ; % COM position vector w.r.t & expressed in local frame
    body(i).CC_EB.tip = [ l_b/n_seg 0 0 ] ; % tip position vector w.r.t & expressed in local frame
    body(i).CC_EB.stiff = [ tspringCoeff 0 0 0 0 rspringCoeff ] ; % curvilinear translational and rotational stiffness: [ t n b tr nr br ]
                                                                  % set to Ea/l, EI/l or GJ/l from EB relation
    body(i).CC_EB.viscdamp = [ tdampViscous 0 0 0 0 rdampViscous ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
    body(i).CC_EB.couldamp = [ tdampCoulomb 0 0 0 0 rdampCoulomb ] ; % curvilinear translational and rotational damping: [ t n b tr nr br ]
    body(i).CC_EB.par = [ inf inf 0 ] ; % put 'inf' for CC_EB states to be dof and values for fixed directions
                                        % [ l theta phi ] for CC_EB, 3*body(i).CC_EB.slice for VC
...
joint(i+1).tr(1).trans = [ l_b/n_seg+l_fs 0 0 ] ; % this is mid-base sensor position in local curvinilinear frame
joint(i+1).tr(1).rot = [ 0 0 ] ;

exload = [] ; % definition
exload(1).body = i ; % point of contact body number
...
subplot(4,5,11) ; plot ( t , z(:,end-1) , 'LineWidth' , 2 ) ;
title('linear elongation states') ; legend( 'l_1' ) ; ylabel( 'l [m]' ) ; xlabel( 't [s]' ) ;
subplot(4,5,13) ; plot ( t , z(:,end) , 'LineWidth' , 2 ) ;
title('curvature states') ; legend( '\theta_1' ) ; ylabel( '\theta [1/m]' ) ; xlabel( 't [s]' ) ;
pause;
...
for i = 1 : n_p
    ...
    tmp = ( T_psi * rjtip(n_seg,4:6,i)' )' ; % tip for 1st body
    rjt3d(:,:,i) = tmp ;
...
legend( 'x_{sim}' , 'y_{sim}' , 'z_{sim}' ) ; ylabel( '[m]' ) ;

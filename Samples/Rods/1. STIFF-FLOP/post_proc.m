...
for i = 1 : numel( t )
    ...
    % error analysis
    step = floor( t(i) / 0.05 ) ;
    Err_vec(i,:) = par.tip_exp(step,2:4) - tip_sim(i,:) ;
...
figcompare = figure ;
subplot(1,2,1)
plot( par.tip_exp(:,1) , par.tip_exp(:,2:4) , '--' , t , tip_sim , par.linetype , 'LineWidth' , 2 ) ;
...
plot( par.tip_exp(:,1) , par.tip_exp(:,2:4) , '--' , t(tmp:end) , tip_sim_filter(tmp:end,:) , par.linetype , 'LineWidth' , 2 ) ;

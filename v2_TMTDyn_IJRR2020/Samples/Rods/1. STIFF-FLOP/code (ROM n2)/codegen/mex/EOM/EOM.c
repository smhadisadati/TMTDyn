         ...
         real_T dz_data[], int32_T dz_size[1])
{
  ...
  i0 = (int32_T)(((b_par->rom.sprdmp[12] - 1.0E-6) + (ds - 1.0E-6)) / ds);
  emlrtForLoopVectorCheckR2012b(1.0E-6, ds, b_par->rom.sprdmp[12] - 1.0E-6,
    mxDOUBLE_CLASS, i0, &d_emlrtRTEI, sp);
  for (s = 0; s < i0; s++) {
    st.site = &i_emlrtRSI;
    b_sprdmpF13(&st, b_par->var, z, 1.0E-6 + (real_T)s * ds, T, fg, y, in);
    for (i = 0; i < 12; i++) {
      alpha1 = 0.0;
      for (k = 0; k < 6; k++) {
        alpha1 += T[i + 12 * k] * y[k];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% single body with logF for variable properties across length: NOT GOOD FOR
% CURVATURE CONTROLLER!!! RESULTS IN NO STRAIGHT SECTION FOR INTRODUCER!!!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% initialize
close all
clear all
...
mu_MC = 1e-12 ; % 1e-16 for exp1, Rayleigh damping M coefficients
mu_KC = 5e-3 ; % 5e-3 for 2inputs & 5e-2 for 3inputs, Rayleigh damping K coefficients

PID = [ 2e6, 0, 0 ] ; % keep D=1! PID gains for the error tracking with the jacobian-inverse controller
...
syms s sigma_s1_ sigma_s2_ E_s1_ E_s2_ G_s1_ G_s2_ ...
    ...
    haptic_x_ haptic_y_ haptic_z_ P_ I_ D_ real
vars = [ sigma_s1_ sigma_s2_ E_s1_ E_s2_ G_s1_ G_s2_ ...
    r_s11_ r_s12_ r_s21_ r_s22_ ...
    mu_KC_ mu_pow_ g_x_ g_y_ g_z_ ...
    l_s1_ l_s2_ l_ex_ dl_ kappa_x_ kappa_y_ ...
    f_tx_ f_ty_ f_tz_ tau_tx_ tau_ty_ tau_tz_ ...
    haptic_x_ haptic_y_ haptic_z_ P_ I_ D_ ] ;

var_vals = [sigma_s(1), sigma_s(2), E_s(1), E_s(2), G_s(1), G_s(2), r_s(1,1), r_s(1,2), r_s(2,1), r_s(2,2), ...
    mu_KC, mu_pow, g_x, g_y, g_z, l_s(1), l_s(2), 0, dl0, 0, 0, zeros(1,6), 0, 0, l_s(1), PID] ;
...
% Logistic fuction 
lfv = [ 1, 1-logfunc(s,l_s2_-dl_) ] ; % -dl for base growing

% radius for animation
r_s_all = [ logfunc(s,l_s2_-dl_), 1-logfunc(s,l_s2_-dl_) ] * r_s_(:,2) ;
    ...
    tmtdyn()...
        .simulation()... % simulation
            ...
            .analysis()...
                ...
                .controller_type('weight_compensation_&_attractor_field_controller')... % {'jacobian_inverse_controller', 'input_output_feedback_linearization', 'weight_compensation_&_attractor_field_controller'}
                .dynamic_sim('generate_m_file', 'external', 0, t_end)... % matlab, sundials_ode, sundials_dae, radau, radau_mex, external
            ...
            .body('growing')...
                ...
                .connected_to_joint()...
                    ...
                    .with_transformation_from_body()...
                        ...
                        .growing('from_base')...
                            .rom_order(0)...
                            ...base_value(dl_)... % geometric constraint: 2 inputs
                            .unknown_direct_input_at_coefficient(1)... % 3 inputs
                    .beam_stiffness()...
                        .coefficient([ K_v_all K_u_all ])...
                        .relaxed_state([0, 0, 1, inf, inf, 0])... % kappa_x & kappa_y relaxed states are unknown control inputs
                    ...beam_damping()...
...
results = results... % to prevent matlab error for complex argument
            .dynamic_inverse_controller('tip_controller')... % tip_controller is an arbitrary name for this controller
                .from_body_tip(1)...
                .desired_trajectory(-[haptic_x_, haptic_y_, haptic_z_])... % constraint is set between ground and this body, so (-) sign is required
                .pid_controller_gains(P_, I_, D_)... % P*q, D*dq, I: not yet implemented

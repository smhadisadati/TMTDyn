...
spring.rom_offset = []; % 1st element: acts after fitting for each dof, affects logistic switch unit curve_length but not the spline knots, 2nd element: act before fitting including spline knots
...
dof.init = empty_sym;
dof.rom_offset = zero_sym * ones(1,2); % 1st element: acts after fitting for each dof, affects logistic switch unit curve_length but not the spline knots, affects rom_series, not when it is used for dof equality constraint or init_rom_series used for spring equality constraint; 2nd element: act before fitting including spline knots

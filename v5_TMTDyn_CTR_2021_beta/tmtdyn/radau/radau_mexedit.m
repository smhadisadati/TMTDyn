...
function [tout, yout] = radau_mexedit(tspan,y0,vars,options)
%
...
ValP = complex(0);  
Dd = 0;
T_1 = complex(zeros(1,1));
TI_1 = complex(zeros(1,1));
C1 = zeros(1,1);
ValP1 = complex(zeros(1,1));
Dd1 = zeros(1,1);
T_3=complex(zeros(3,3));
TI_3 = complex(zeros(3,3));
C3 = zeros(3,1);
ValP3 = complex(zeros(3,1));
Dd3 = zeros(3,1);
T_5 = complex(zeros(5,5));
TI_5 = complex(zeros(5,5));
C5 = zeros(5,1);
ValP5 = complex(zeros(5,1));
Dd5 = zeros(5,1);
T_7 = complex(zeros(7,7));
TI_7 = complex(zeros(7,7));
C7 = zeros(7,1);
ValP7 = complex(zeros(7,1));
Dd7 = zeros(7,1);
L = complex(zeros(Ny, Ny, 7));
U = complex(zeros(Ny, Ny, 7));
P = complex(zeros(Ny, Ny, 7));
...
if (length(Op.AbsTol) ~= Ny) && (length(Op.AbsTol) ~=1)
  error([Solver_Name, ': AbsTol vector of length 1 or %d.']);
  Ny
end
...
if (length(Op.RelTol) ~= Ny) && (length(Op.RelTol) ~=1)
  error([Solver_Name, ': RelTol vector of length 1 or %d.']);
  Ny
end
...
elseif Op.MaxNbrStep <= 0
  error([Solver_Name,': Wrong input "MaxNbrStep", must be > 0']);
end
% ------- MASS  
...
elseif Op.NbrInd1 < 0
  error([Solver_Name,': Wrong input "NbrInd1", must be > 0']);
elseif Op.NbrInd1 == 0
  Op.NbrInd1 = Ny;  
end
...
elseif Op.NbrInd2 < 0
  error([Solver_Name,': Wrong input "NbrInd2", must be > 0']);
end
if ~isnumeric(Op.NbrInd3)
  error([Solver_Name,': Wrong input "NbrInd3" must be a number >= 0']);
elseif Op.NbrInd3 < 0
  error([Solver_Name,': Wrong input "NbrInd3", must be > 0']);
end
...
elseif abs(Op.JacRecompute) >= 1
  error([Solver_Name, ': Curious input for "JacRecompute" ']);
  Op.JacRecompute
end
...
elseif Op.MaxNbrNewton < 4
  error([Solver_Name,': Wrong input "MaxNbrNewton", must be > 4']);
end  
...
elseif Op.Safe <= 0.001 || Op.Safe >= 1 
  error([Solver_Name,': Curious input for Safe, must be in ]0.001 .. 1[ ']);
end
...
elseif Op.Quot1 > 1
  error([Solver_name,': Curious input for Quot1, must be in <= 1']);
end
if ~isnumeric(Op.Quot2)
  error([Solver_Name, ': Wrong input "Quot2" must be numeric ']);
elseif Op.Quot2 < 1
  error([Solver_name,': Curious input for Quot2, must be  > 1']);
end
...
[tout,yout] = radausolver(tspan,y0,Op,vars,pars);
...
function [tout, yout] = radausolver(tspan,y,Op,vars,pars)
...
InitNbrStg = true;
pars = Coertv(MinNbrStg,MaxNbrStg,RealYN,InitNbrStg,pars);
InitNbrStg = false;
pars = Coertv(NbrStg,NbrStg,RealYN,InitNbrStg,pars);
...
erracc = 0;
Jac = (zeros(Ny, Ny));       
z = complex(zeros(Ny,NbrStg));
w = complex(zeros(Ny,NbrStg));
...
%       end
      Jac = NumJac(t,y,vars);   % OdeFcn needs parameters
      if any(isnan(Jac))
...
% ------- COMPUTE THE MATRICES E1 AND E2 AND THEIR DECOMPOSITIONS L U P 
  if NeedNewLU                                           
    [U_Sing, pars] = DecomRC(h,pars.ValP,Mass,Jac,RealYN,pars);
    ...
    if U_Sing > 0                    % GOTO 78
      ...
      N_Sing       = N_Sing + 1;
      if N_Sing >= 5
        error([Solver_Name,': Matrix is repeatedly singular at stage ']);
        U_Sing
      end
      h                   = h*0.5;
      hhfac               = 0.5;
  ...
  if ( First || Start_Newt || ChangeFlag )             
    ...
    cq     = pars.C*hquot;            
    for q = 1:NbrStg
      z(:,q) = (cq(q)-pars.C(1)+1)* cont(:,NbrStg);
      for q1 = 2:NbrStg
        z(:,q) = (z(:,q) + cont(:,NbrStg+1-q1))*(cq(q)-pars.C(q1)+1);
      end
    end    
    for n = 1:Ny                 %   w <-> FF   cont c <-> AK   cq(1) <-> C1Q  
      w(n,:) = pars.TI(1,:) * z(n,1);
      for q = 2 : NbrStg
        w(n,:) = w(n,:) + pars.TI(q,:)*z(n,q);
      end                      
    end 
...
%       if Arg.Ode
%         f(:,q) = feval(OdeFcn,t+pars.C(q)*h, y+z(:,q),varargin{:}); % OdeFcn needs parameters
%       else
%         f(:,q) = feval(OdeFcn,t+pars.C(q)*h, y+z(:,q));         % OdeFcn needs no parameters
%       end
      f(:,q) = EOM(t+pars.C(q)*h, y+real(z(:,q)),vars); % OdeFcn needs parameters
      if any(isnan(f))
        error([Solver_Name, ': Some components are NAN']);        
      end
    end
    ...
    for n = 1:Ny
      z(n,:) = pars.TI(1,:)* f(n,1);
      for q = 2:NbrStg
        z(n,:) = z(n,:) +pars.TI(q,:)* f(n,q);
      end
    end
% ------- SOLVE THE LINEAR SYSTEMS    % Ligne 1037                    
    [z, pars]       = Solvrad(z,w,pars.ValP,h,Mass,RealYN,pars); 
...
%                           First, Reject,Stat,OdeFcn,t,y,varargin{:});
  [err,Stat,pars] = Estrad(z,pars.Dd,h,Mass,Scal,f0, ...
                          First, Reject,Stat,t,y,vars,pars);
...
% ------- PREDICTIVE CONTROLLER OF GUSTAFSSON  
    ...
    cont(:,NbrStg) = real(z(:,1))/pars.C(1);
    for q = 1:NbrStg-1
      Fact      = 1/(pars.C(NbrStg-q) - pars.C(NbrStg-q+1));
      cont(:,q) = real(z(:,NbrStg-q) - z(:,NbrStg-q+1))*Fact;
    end
    for jj = 2:NbrStg
      for k = NbrStg:-1:jj
        if NbrStg - k ==0
          ...
          Fact = 1/(pars.C(NbrStg-k) - pars.C(NbrStg - k + jj) );
        end
...
%     if nargout > 0
      switch OutFlag        
        ...
        case 2          % Computed points, with refinement 
          ...
          ii           = oldnout+1:nout-1;
          tinterp      = t+h*S - h;
          [yinterp,pars]      = ntrprad(tinterp,t,y,h,cont,pars);
          tout(ii)     = tinterp;
        ...
        case 3          % Output only at tspan points
          ...
          while next <= ntspan
            ...
            [yinterp, pars]      = ntrprad(tspan(next),t,y,h,cont,pars);
...
%       f0 = feval(OdeFcn,t,y);            % OdeFcn needs no parameters
%     end
    f0 = EOM(t,y,vars);    % OdeFcn needs parameters 
    if any(isnan(f0))
      error([Solver_Name, ': Some components of are NAN']);      
    end
    Stat.FcnNbr = Stat.FcnNbr+1;
...
function [yinterp, pars] = ntrprad(tinterp,t,y,h,cont,pars)
% global C
...
function  [U_Sing, pars] = DecomRC(h,ValP,Mass,Jac,RealYN,pars)
% global L U P 
...
if RealYN 
  ...
  B                =  valp(1)*Mass-Jac;
  B = full(B);
  [pars.L(:,:,1),pars.U(:,:,1),pars.P(:,:,1)]=lu(B);   
  if NbrStg > 1
    for q = 1:(NbrStg-1)/2
      q1                  = q+1;
      q2                  = 2*q;
      ...
      B = full(B);
      [pars.L(:,:,q1),pars.U(:,:,q1),pars.P(:,:,q1)]=lu(B);
    end
  end           
else   % complex
  NuMax = NbrStg;
  for q = 1:NbrStg      
    temp = full( valp(q)*Mass-Jac );
    [pars.L(:,:,q),pars.U(:,:,q),pars.P(:,:,q)] = lu( temp );
  end
end

U_Sing = 0;
for q = 1:NuMax
  if (any(abs(diag(pars.U(:,:,q))))<eps) 
...
function [z, pars] = Solvrad(z,w,ValP,h,Mass,RealYN,pars)
...
if RealYN
  ...
  z(:,1) = pars.U(:,:,1)\(pars.L(:,:,1)\(pars.P(:,:,1)*z(:,1)));
  if NbrStg > 1
    for q = 1:(NbrStg-1)/2
      q1 = q+1;
      q2 = 2*q;
      ...
      z2 = pars.U(:,:,q1)\(pars.L(:,:,q1)\(pars.P(:,:,q1)*(z2+1i*z3)));
      z(:,q2) = real(z2);
      z(:,q3) = imag(z2);
    end
  end
  ...
  for q = 1:NbrStg     
    z(:,q) = z(:,q)-valp(q)*Mw(:,q);
    z(:,q) = pars.U(:,:,q)\(pars.L(:,:,q)\(pars.P(:,:,q)*z(:,q)));
  end
end
return
...
%                                  First, Reject,Stat,...
%                                  OdeFcn,t,y,varargin)
function [err,Stat,pars] = Estrad(z,Dd,h,Mass,Scal,f0, ...
                                 First, Reject,Stat,...
                                 t,y,vars,pars)  
% global U L P 
...
err_v = pars.U(:,:,1)\(pars.L(:,:,1)\(pars.P(:,:,1)*(f0+temp)));
...
%   end
  err_v = EOM(t,y+err,vars);
  Stat.FcnNbr = Stat.FcnNbr + 1;  
  err_v = pars.U(:,:,1)\(pars.L(:,:,1)\(pars.P(:,:,1)*(err_v+temp)));
  err = norm(err_v./Scal,2);
  err = max(err/SqrtNy,1e-10);  
end
...
if ~isempty(tout)
  [yinterp, pars] = ntrprad(tout,t,y,h,cont,pars);
...
function pars = Coertv(MinNbrStg,MaxNbrStg,RealYN,Init,pars)
...
if Init
  for NbrStg = MinNbrStg:MaxNbrStg
    switch NbrStg
      case 1
        pars = Coertv1(RealYN,pars);
      case 3
        pars = Coertv3(RealYN,pars);
      case 5
        pars = Coertv5(RealYN,pars);
      case 7
        pars = Coertv7(RealYN,pars);
    end
  end
else
  ...
  switch NbrStg
    case 1
      pars.T = pars.T_1; pars.TI = pars.TI_1; pars.ValP = pars.ValP1; pars.C = pars.C1; pars.Dd = pars.Dd1;
    case 3
      pars.T = pars.T_3; pars.TI = pars.TI_3; pars.ValP = pars.ValP3; pars.C = pars.C3; pars.Dd = pars.Dd3;
    case 5
      pars.T = pars.T_5; pars.TI = pars.TI_5; pars.ValP = pars.ValP5; pars.C = pars.C5; pars.Dd = pars.Dd5;
    case 7
      pars.T = pars.T_7; pars.TI = pars.TI_7; pars.ValP = pars.ValP7; pars.C = pars.C7; pars.Dd = pars.Dd7;
  end
end
return

function pars = Coertv1(RealYN,pars)
...
function pars = Coertv3(RealYN,pars)
...
Sq6   = sqrt(6);
pars.C3(1) = (4.0 - Sq6)/10.0;
pars.C3(2) = (4.0 + Sq6)/10.0;
pars.C3(3) = 1;
...
if RealYN
  ...
  pars.ValP3(1) = 1.0/ValP(1);
  pars.ValP3(2) = ValP(2)/Cno;
  pars.ValP3(3) = ValP(3)/Cno;    
else  
  % A, matrix construction from the C values (Hairer .. page 78) 
  CP3 = [1  pars.C3(1) pars.C3(1)^2 
         1  pars.C3(2) pars.C3(2)^2 
         1  pars.C3(3) pars.C3(3)^2 ];
     
  CQ3 = [pars.C3(1) pars.C3(1)^2/2  pars.C3(1)^3/3 
         pars.C3(2) pars.C3(2)^2/2  pars.C3(2)^3/3 
         pars.C3(3) pars.C3(3)^2/2  pars.C3(3)^3/3 ];
...
pars.T_3(1,:) = [T3(1,1),T3(2,1),T3(3,1)];
pars.T_3(2,:) = [T3(1,2),T3(2,2),T3(3,2)];
pars.T_3(3,:) = [T3(1,3),T3(2,3),T3(3,3)]; 

pars.TI_3(1,:) = [TI3(1,1),TI3(2,1),TI3(3,1)];
pars.TI_3(2,:) = [TI3(1,2),TI3(2,2),TI3(3,2)];
pars.TI_3(3,:) = [TI3(1,3),TI3(2,3),TI3(3,3)];

return

function pars = Coertv5(RealYN,pars) 
...
pars.C5(1)  =  0.5710419611451768219312e-01;
pars.C5(2)  =  0.2768430136381238276800e+00;
pars.C5(3)  =  0.5835904323689168200567e+00;
pars.C5(4)  =  0.8602401356562194478479e+00;
pars.C5(5)  =  1.0;

pars.Dd5(1) = -0.2778093394406463730479D+02;
pars.Dd5(2) =  0.3641478498049213152712D+01;
pars.Dd5(3) = -0.1252547721169118720491D+01;
pars.Dd5(4) =  0.5920031671845428725662D+00;
pars.Dd5(5) = -0.2000000000000000000000D+00;
pars.Dd5    = pars.Dd5(:);
...
if RealYN
  ...
  pars.ValP5(1) =  0.6286704751729276645173D+01;  % U1 in Fortran
  pars.ValP5(2) =  0.3655694325463572258243D+01;  % Ce sont les val propres
  pars.ValP5(3) =  0.6543736899360077294021D+01;  % de inv(A)
  pars.ValP5(4) =  0.5700953298671789419170D+01;
  pars.ValP5(5) = 0.3210265600308549888425D+01; 
  % Le signe - ci-dessus n'existe pas dans la version fortran.
  % Il amliore le changement de "stage" dans Robertson
  
else
  
  CP5  = [1  pars.C5(1) pars.C5(1)^2 pars.C5(1)^3 pars.C5(1)^4
          1  pars.C5(2) pars.C5(2)^2 pars.C5(2)^3 pars.C5(2)^4
          1  pars.C5(3) pars.C5(3)^2 pars.C5(3)^3 pars.C5(3)^4
          1  pars.C5(4) pars.C5(4)^2 pars.C5(4)^3 pars.C5(4)^4
          1  pars.C5(5) pars.C5(5)^2 pars.C5(5)^3 pars.C5(5)^4];
      
  CQ5 = [pars.C5(1) pars.C5(1)^2/2 pars.C5(1)^3/3 pars.C5(1)^4/4 pars.C5(1)^5/5
         pars.C5(2) pars.C5(2)^2/2 pars.C5(2)^3/3 pars.C5(2)^4/4 pars.C5(2)^5/5
         pars.C5(3) pars.C5(3)^2/2 pars.C5(3)^3/3 pars.C5(3)^4/4 pars.C5(3)^5/5
         pars.C5(4) pars.C5(4)^2/2 pars.C5(4)^3/3 pars.C5(4)^4/4 pars.C5(4)^5/5
         pars.C5(5) pars.C5(5)^2/2 pars.C5(5)^3/3 pars.C5(5)^4/4 pars.C5(5)^5/5];
...
pars.T_5(1,:) = [T5(1,1),T5(2,1),T5(3,1),T5(4,1),T5(5,1)];
pars.T_5(2,:) = [T5(1,2),T5(2,2),T5(3,2),T5(4,2),T5(5,2)];
pars.T_5(3,:) = [T5(1,3),T5(2,3),T5(3,3),T5(4,3),T5(5,3)];
pars.T_5(4,:) = [T5(1,4),T5(2,4),T5(3,4),T5(4,4),T5(5,4)];
pars.T_5(5,:) = [T5(1,5),T5(2,5),T5(3,5),T5(4,5),T5(5,5)];

pars.TI_5(1,:) = [TI5(1,1),TI5(2,1),TI5(3,1),TI5(4,1),TI5(5,1)];
pars.TI_5(2,:) = [TI5(1,2),TI5(2,2),TI5(3,2),TI5(4,2),TI5(5,2)];
pars.TI_5(3,:) = [TI5(1,3),TI5(2,3),TI5(3,3),TI5(4,3),TI5(5,3)];
pars.TI_5(4,:) = [TI5(1,4),TI5(2,4),TI5(3,4),TI5(4,4),TI5(5,4)];
pars.TI_5(5,:) = [TI5(1,5),TI5(2,5),TI5(3,5),TI5(4,5),TI5(5,5)];
return

function pars = Coertv7(RealYN,pars)
% See page 78 in Stiff differential Equations:  Radau IIa order 5
% Butcher representation
%  C(1) |  A(1,1)  A(1,2)  A(1,3)
%  C(1) |  A(2,1)  A(2,2)  A(2,3)
...
pars.C7(1) = 0.2931642715978489197205D-01;
pars.C7(2) = 0.1480785996684842918500D+00;
pars.C7(3) = 0.3369846902811542990971D+00;
pars.C7(4) = 0.5586715187715501320814D+00;
pars.C7(5) = 0.7692338620300545009169D+00;
pars.C7(6) = 0.9269456713197411148519D+00;
pars.C7(7) = 1;

pars.Dd7(1) = -0.5437443689412861451458D+02;
pars.Dd7(2) =  0.7000024004259186512041D+01;
pars.Dd7(3) = -0.2355661091987557192256D+01;
pars.Dd7(4) =  0.1132289066106134386384D+01;
pars.Dd7(5) = -0.6468913267673587118673D+00;
pars.Dd7(6) =  0.3875333853753523774248D+00;
pars.Dd7(7) = -0.1428571428571428571429D+00;
pars.Dd7    = pars.Dd7(:);
...
if RealYN
  ...
  pars.ValP7(1) =  0.8936832788405216337302D+01;     % U1 dans le code
  pars.ValP7(2) =  0.4378693561506806002523D+01;
  pars.ValP7(3) =  0.1016969328379501162732D+02;
  pars.ValP7(4) =  0.7141055219187640105775D+01;
  pars.ValP7(5) =  0.6623045922639275970621D+01;
  pars.ValP7(6) =  0.8511834825102945723051D+01;
  pars.ValP7(7) =  0.3281013624325058830036D+01;
else
  CP7 = [1  pars.C7(1) pars.C7(1)^2 pars.C7(1)^3 pars.C7(1)^4 pars.C7(1)^5 pars.C7(1)^6
         1  pars.C7(2) pars.C7(2)^2 pars.C7(2)^3 pars.C7(2)^4 pars.C7(2)^5 pars.C7(2)^6
         1  pars.C7(3) pars.C7(3)^2 pars.C7(3)^3 pars.C7(3)^4 pars.C7(3)^5 pars.C7(3)^6
         1  pars.C7(4) pars.C7(4)^2 pars.C7(4)^3 pars.C7(4)^4 pars.C7(4)^5 pars.C7(4)^6
         1  pars.C7(5) pars.C7(5)^2 pars.C7(5)^3 pars.C7(5)^4 pars.C7(5)^5 pars.C7(5)^6
         1  pars.C7(6) pars.C7(6)^2 pars.C7(6)^3 pars.C7(6)^4 pars.C7(6)^5 pars.C7(6)^6
         1  pars.C7(7) pars.C7(7)^2 pars.C7(7)^3 pars.C7(7)^4 pars.C7(7)^5 pars.C7(7)^6];
                 
  CQ7 = [pars.C7(1) pars.C7(1)^2/2 pars.C7(1)^3/3 pars.C7(1)^4/4 pars.C7(1)^5/5 pars.C7(1)^6/6 pars.C7(1)^7/7
         pars.C7(2) pars.C7(2)^2/2 pars.C7(2)^3/3 pars.C7(2)^4/4 pars.C7(2)^5/5 pars.C7(2)^6/6 pars.C7(2)^7/7
         pars.C7(3) pars.C7(3)^2/2 pars.C7(3)^3/3 pars.C7(3)^4/4 pars.C7(3)^5/5 pars.C7(3)^6/6 pars.C7(3)^7/7
         pars.C7(4) pars.C7(4)^2/2 pars.C7(4)^3/3 pars.C7(4)^4/4 pars.C7(4)^5/5 pars.C7(4)^6/6 pars.C7(4)^7/7
         pars.C7(5) pars.C7(5)^2/2 pars.C7(5)^3/3 pars.C7(5)^4/4 pars.C7(5)^5/5 pars.C7(5)^6/6 pars.C7(5)^7/7
         pars.C7(6) pars.C7(6)^2/2 pars.C7(6)^3/3 pars.C7(6)^4/4 pars.C7(6)^5/5 pars.C7(6)^6/6 pars.C7(6)^7/7
         pars.C7(7) pars.C7(7)^2/2 pars.C7(7)^3/3 pars.C7(7)^4/4 pars.C7(7)^5/5 pars.C7(7)^6/6 pars.C7(7)^7/7];
...
pars.T_7(1,:) = [T7(1,1),T7(2,1),T7(3,1),T7(4,1),T7(5,1),T7(6,1),T7(7,1)];
pars.T_7(2,:) = [T7(1,2),T7(2,2),T7(3,2),T7(4,2),T7(5,2),T7(6,2),T7(7,2)];
pars.T_7(3,:) = [T7(1,3),T7(2,3),T7(3,3),T7(4,3),T7(5,3),T7(6,3),T7(7,3)];
pars.T_7(4,:) = [T7(1,4),T7(2,4),T7(3,4),T7(4,4),T7(5,4),T7(6,4),T7(7,4)];
pars.T_7(5,:) = [T7(1,5),T7(2,5),T7(3,5),T7(4,5),T7(5,5),T7(6,5),T7(7,5)];
pars.T_7(6,:) = [T7(1,6),T7(2,6),T7(3,6),T7(4,6),T7(5,6),T7(6,6),T7(7,6)];
pars.T_7(7,:) = [T7(1,7),T7(2,7),T7(3,7),T7(4,7),T7(5,7),T7(6,7),T7(7,7)];

pars.TI_7(1,:) = [TI7(1,1),TI7(2,1),TI7(3,1),TI7(4,1),TI7(5,1),TI7(6,1),TI7(7,1)];
pars.TI_7(2,:) = [TI7(1,2),TI7(2,2),TI7(3,2),TI7(4,2),TI7(5,2),TI7(6,2),TI7(7,2)];
pars.TI_7(3,:) = [TI7(1,3),TI7(2,3),TI7(3,3),TI7(4,3),TI7(5,3),TI7(6,3),TI7(7,3)];
pars.TI_7(4,:) = [TI7(1,4),TI7(2,4),TI7(3,4),TI7(4,4),TI7(5,4),TI7(6,4),TI7(7,4)];
pars.TI_7(5,:) = [TI7(1,5),TI7(2,5),TI7(3,5),TI7(4,5),TI7(5,5),TI7(6,5),TI7(7,5)];
pars.TI_7(6,:) = [TI7(1,6),TI7(2,6),TI7(3,6),TI7(4,6),TI7(5,6),TI7(6,6),TI7(7,6)];
pars.TI_7(7,:) = [TI7(1,7),TI7(2,7),TI7(3,7),TI7(4,7),TI7(5,7),TI7(6,7),TI7(7,7)];
return

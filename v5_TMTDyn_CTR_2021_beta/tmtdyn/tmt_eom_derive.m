...
for i_joint = 1 : n_jonts % look into all joints
    ...
    for i_copies = 1 : joint(i_joint).n_copy % iterate over the copies of each joint
        ...
        if joint(i_joint).rom.isROM % only for reduced order model
            ...
            if joint(i_joint).rom.length(i_copies,4) ~= 0 % length: [init_length_start, init_length_end, type, coeff, i_joint, i_copy, i_dofAxis, i_coeff, length]

                temp = joint(i_joint).rom.length(i_copies,5:end) ; % [i_joint, i_copy, i_dofAxis, i_coeff, length]
        ...
        if joint(i_joint).second(1,1) == b_count % if the joint connects this body to another one,
            ...
            for i_copies = 1 : body(b_count).n_copy
                ...
                if ~joint(i_joint).rom.isROM % not a reduced order model
                    ...
                    s1 = s * ( body(b_count).rom.length(i_copies,2) - body(b_count).rom.length(i_copies,1) ) + body(b_count).rom.length(i_copies,1) ; % normalized curve length

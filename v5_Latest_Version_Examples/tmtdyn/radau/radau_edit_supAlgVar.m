...
function [tout, yout] = radau_edit_supAlgVar(odefun,tspan,y0,vars,options)
%
...
[m,n]           = size(f0);
if n > 1
    error([Solver_Name,':  Function odefun(t,y) must return a column vector.']);
elseif m ~= Ny
    error([Solver_Name,':  Vector odefun(t,y) must be same length as initial conditions.']);
end
...
while (Stat.StepNbr <= MaxNbrStep && PosNeg*t < PosNeg*tfinal) % ligne 849 fortran
    ...
    if abs(h) <= hmin            % h et hmin are positive
            fprintf(['Radau:Failure at t=%e.  Unable to meet integration ' ...
    ...
    if NeedNewJac
        ...
        % else
            ...
            %       end
            Jac = NumJac(odefun,t,y,vars,Ny);   % OdeFcn needs parameters
            if any(isnan(Jac))
    ...
    [err,Stat] = Estrad(odefun,z,Dd,h,Mass,Scal,f0, ...
        First, Reject,Stat,t,y,vars,nAlgVars);
...
function Jac = NumJac(odefun,t,y,vars,Ny)
% Fcn_Name = 'NumJac';
% Arg.Ode =  abs(nargin(OdeFcn)) > 2;
% if Arg.Ode
...
% if any(isnan(yPrime0))
    ...
    % %     FcnError  = OdeFcn;
    % %   end
    % error('isnan(yPrime0): Some components are NAN');
% end
...
% err_v = err_v(1:end/2-nAlgVars); % vel.s
% Scal = Scal(1:end/2-nAlgVars);
err_v = err_v(end/2+1:end-nAlgVars); % acc.s
Scal = Scal(end/2+1:end-nAlgVars);
...
if  First || Reject
    ...
    %   err_v = err_v(1:end/2-nAlgVars); % vel.s
    err_v = err_v(end/2+1:end-nAlgVars); % acs.s
    err = norm(err_v./Scal,2);
    err = max(err/SqrtNy,1e-10);
end

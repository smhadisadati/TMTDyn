classdef rom_builder < handle
    ...
    methods
		...
		function self = varies_with_dof(self, coeff, i_joint, i_dofAxis, i_copy) % note order change
			if nargin == 3
				i_copy = 1 * ones(numel(coeff(:,1)), 1); % results in all mesh nodes having the same equal2 property
			end
            temp = ones(self.source.n_copy,1) .* [coeff, i_joint, i_copy, i_dofAxis, ones(numel(coeff(:,1)), 1), zeros(numel(coeff(:,1)), 1)];
            for i = 1 : self.source.n_copy % for mesh
		...
		function self = varies_with_dof_order(self, coeff, i_joint, i_dofAxis, i_coeff, i_copy) % note the order change
            if nargin == 5
                i_copy = 1 * ones(numel(coeff(:,1)), 1); % results in all mesh nodes having the same equal2 property
            end
            temp = ones(self.source.n_copy,1) .* [coeff, i_joint, i_copy, i_dofAxis, i_coeff, zeros(numel(coeff(:,1)), 1)];
            for i = 1 : self.source.n_copy % for mesh
                self.pipe.length(i,4:end) = temp(i,:);
            end
            self.length_val = self.pipe.length;
		...
		function self = varies_with_dof_at_axial_location(self, coeff, i_joint, i_dofAxis, curve_length, i_copy) % note the order change
			if nargin == 4
				curve_length = 0 * ones(numel(coeff(:,1)), 1); % equal to the other joint/dof at base
			end
			if nargin == 5
				i_copy = 1 * ones(numel(coeff(:,1)), 1); % results in all mesh nodes having the same equal2 property
			end
            temp = ones(self.source.n_copy,1) .* [coeff, i_joint, i_copy, i_dofAxis, zeros(numel(coeff(:,1)), 1), curve_length];
            for i = 1 : self.source.n_copy % for mesh
                self.pipe.length(i,4:end) = temp(i,:);
            end
            self.length_val = self.pipe.length;
		...
		function self = varies_with_dof_at_tip(self, coeff, i_joint, i_dofAxis, curve_length, i_copy) % note the order change
			if nargin == 4
				curve_length = NAN * ones(numel(coeff(:,1)), 1); % equal to the other joint/dof at tip
			end
			if nargin == 5
				i_copy = 1 * ones(numel(coeff(:,1)), 1); % results in all mesh nodes having the same equal2 property
			end

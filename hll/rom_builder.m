classdef rom_builder < handle
    ...
    properties (GetAccess = public, SetAccess = private)
        ...
        default_order_val;
        type_val;
        length_val; % [l_start_rom, l_end_rom, type, coeff(-+1), i_j(joint), i_d(mesh), i_h(DOF), i_r(order coeff in rom), l(length along rom)]
		base_condition_val;
		free_base_val;
		init_s_val;
		fit_type_val;
    ...
    methods
        ...
        function self = growing_from(self, type) % {'none', 'tip_growing', 'base_growing', 'sliding'}
            if nargin == 1
                type = 'none';
            end
            switch type
            ...
            type = ones(self.source.n_mesh,1) .* type;
            self.type_val = type;
        ...
        function self = length(self, length_ie, length_e) % integration range along length
            ...
            length = ones(self.source.n_mesh,1) .* length;
		...
		function self = varies_with_dof(self, coeff, i_j, i_h, i_d) % note order change
            ...
            temp = ones(self.source.n_mesh,1) .* [coeff, i_j, i_d, i_h, ones(numel(coeff(:,1)), 1), zeros(numel(coeff(:,1)), 1)];
            for i = 1 : self.source.n_mesh % for mesh
		...
		function self = varies_with_dof_order(self, coeff, i_j, i_h, i_r, i_d) % note the order change
            ...
            temp = ones(self.source.n_mesh,1) .* [coeff, i_j, i_d, i_h, i_r, zeros(numel(coeff(:,1)), 1)];
            for i = 1 : self.source.n_mesh % for mesh
                self.pipe.length(i,4:end) = temp(i,:);
            end
            self.length_val = self.pipe.length;
		...
		function self = varies_with_dof_at_axial_location(self, coeff, i_j, i_h, l, i_d) % note the order change
            ...
            temp = ones(self.source.n_mesh,1) .* [coeff, i_j, i_d, i_h, zeros(numel(coeff(:,1)), 1), l];
            for i = 1 : self.source.n_mesh % for mesh
                self.pipe.length(i,4:end) = temp(i,:);
            end
            self.length_val = self.pipe.length;
        ...
        function assume_similar_spline_knots(self)
            self.simpKnot = 1;

	...
	.robot(name)...
		...
		.joint_with_repeated_elements(name, mesh_no)... % >> joint()
			.rom()...
				...
				.varies_with_dof(coeff,i_joint, i_dofAxis, i_copy)... % alternativly use trans_rom_axial() and then dof geom. const. routines
				.varies_with_dof_order(coeff, i_joint, i_dofAxis, i_coeff, i_copy)... % alternativly use trans_rom_axial() and then dof geom. const. routines
				.varies_with_dof_at_axial_location(coeff, i_joint, i_dofAxis, curve_length, i_copy)... % alternativly use trans_rom_axial() and then dof geom. const. routines
				.varies_with_dof_at_tip(coeff, i_joint, i_dofAxis, i_copy)... % alternativly use trans_rom_axial() and then dof geom. const. routines
			...
			.with_transformation_from_body(body_no, mesh_no, axial_loc)... % >> aka transformation(), pass no arguments for transformation from ground
				...
				.dof(i_q)... % i_q = dof numerator in each transformation tr
					...
					.set_rom_coefficient(n_coeff, value)...
					...
					.element_set_rom_coefficient(n_copy, n_coeff, value)...
					...
					.direct_input_at_coefficient(n_coeff, input_type, input_val)... % input_type in {'force', 'trajectory'}, set 'trajectory' for constraint w Lagrang multiplier then input_val is acc. or PID
                    .desired_state_trajectory(input_vec)... % inverse dynamic controller, with no Lagrange multiplier, input_val is acc. or PID
                    .desired_state_trajectory_at_coefficient(n_coeff, input_vec)... % inverse dynamic controller, with no Lagrange multiplier, input_val is acc. or PID
					.unknown_direct_input()...
					.unknown_direct_input_at_coefficient(n_coeff)...
                    .state_pid_controller_gains(P_gains_vec, I_gains_vec, D_gains_vec)... % pid gains (w.r.t. velocity) for both constraint and controller, pid_gains_vec: n_copy x 1
                    .state_pid_controller_gains_at_coefficient(n_coeff, P_gains_vec, I_gains_vec, D_gains_vec)... % pid gains (w.r.t. velocity) for both constraint and controller, pid_gains_vec: n_copy x 1
            ...
            .pid_controller_gains(P_gains_vec, I_gains_vec, D_gains_vec)... % pid gains (w.r.t. velocity) for both constraint and controller, pid_gains_vec: n_copy x 6
            .pid_controller_gains_at_strain(n_coeff, P_gains_vec, I_gains_vec, D_gains_vec)... % pid gains (w.r.t. velocity) for both constraint and controller, pid_gains_vec: n_copy x 6
			...
			.beam_stiffness()... % aka spring
				...
				.relaxed_state(init_vals)... % relaxed state: spring initial curve_length or beam initial strains/curvatures/torsion, set inf to assume unknown for controller design
				...
				.set_relaxed_state_rom_coefficient(n_coeff, value)...
				...
				.element_set_relaxed_state_rom_coefficient(n_copy, n_coeff, value)...

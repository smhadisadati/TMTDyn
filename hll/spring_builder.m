classdef spring_builder < handle
    ...
    methods
		...
		function self = boundary_condition(self,n_segm, n_coeff, equal2) % [j_count, n_copy, nj_dof, i_dof_mult, val_or_l_rom, diff_order, coeff] x i_dof_mult -or- [n_j, n_copy, n_h, i_coeff, curve_length, i_p, c] x n_coeff
		...
		function self = set_relaxed_state_rom_coefficient(self, n_coeff, value)
            ...
            self = element_set_relaxed_state_rom_segment_coefficient(self, n_copy, n_segm, n_coeff, value);
        end
        
		function self = set_relaxed_state_rom_segment_coefficient(self, n_segm, n_coeff, value)
            n_copy = 1 : numel(value);
            self = element_set_relaxed_state_rom_segment_coefficient(self, n_copy, n_segm, n_coeff, value);
        end
		...
		function self = element_set_relaxed_state_rom_segment_coefficient(self, n_copy, n_segm, n_coeff, value)
			for i_coeff = n_coeff
                for i_segm = n_segm
                    self.equal2_val{i_segm}{i_coeff}(n_copy,:) = [nan*ones(numel(n_copy),1), zeros(numel(n_copy),3), value.*ones(numel(n_copy),1), zeros(numel(n_copy),1), ones(numel(n_copy),1)]; % ground fixed: similar to dof.set_rom_coefficient
                    self.pipe.equal2{i_segm}{i_coeff}(n_copy,:) = [nan*ones(numel(n_copy),1), zeros(numel(n_copy),3), value.*ones(numel(n_copy),1), zeros(numel(n_copy),1), ones(numel(n_copy),1)];
                end
            end
            self.i_S = self.i_S + 1;
		end
		...
		function self = element_relaxed_state_coefficient_similar_to_dof_order(self, n_copy, n_coeff, coeff, i_joint, i_dofAxis, i_coeff, i_copy) % note the order change
			...
			self.equal2_val{1}{n_coeff}(n_copy,:) = [ones(numel(n_copy),1).*i_joint, ones(numel(n_copy),1).*i_copy, ones(numel(n_copy),1).*i_dofAxis, ones(numel(n_copy),1).*i_coeff, zeros(numel(n_copy),2), ones(numel(n_copy),1).*coeff];
            self.pipe.equal2{1}{n_coeff}(n_copy,:) = [ones(numel(n_copy),1).*i_joint, ones(numel(n_copy),1).*i_copy, ones(numel(n_copy),1).*i_dofAxis, ones(numel(n_copy),1).*i_coeff, zeros(numel(n_copy),2), ones(numel(n_copy),1).*coeff];
            self.i_S = self.i_S + 1;
        end
		...
		function self = element_relaxed_state_coeff_similar_to_dof_at_axial_location(self, n_copy, n_coeff, coeff, i_joint, i_dofAxis, curve_length, i_copy) % note the order change
			...
			self.equal2_val{1}{n_coeff}(n_copy,:) = [ones(numel(n_copy),1).*i_joint, ones(numel(n_copy),1).*i_copy, ones(numel(n_copy),1).*i_dofAxis, zeros(numel(n_copy),1), ones(numel(n_copy),1).*curve_length, zeros(numel(n_copy),1), ones(numel(n_copy),1).*coeff];
            self.pipe.equal2{1}{n_coeff}(n_copy,:) = [ones(numel(n_copy),1).*i_joint, ones(numel(n_copy),1).*i_copy, ones(numel(n_copy),1).*i_dofAxis, zeros(numel(n_copy),1), ones(numel(n_copy),1).*curve_length, zeros(numel(n_copy),1), ones(numel(n_copy),1).*coeff];
            self.i_S = self.i_S + 1;
		end
		...
		function self = element_relaxed_state_base_value(self, n_copy, value)
			self.equal2_val{1}{1}(n_copy,:) = [nan*ones(numel(n_copy),1), zeros(numel(n_copy),3), ones(numel(n_copy),1).*value, zeros(numel(n_copy),1), ones(numel(n_copy),1)];
            self.pipe.equal2{1}{1}(n_copy,:) = [nan*ones(numel(n_copy),1), zeros(numel(n_copy),3), ones(numel(n_copy),1).*value, zeros(numel(n_copy),1), ones(numel(n_copy),1)];
            self.i_S = self.i_S + 1;
        end
		...
		function self = element_relaxed_state_base_director(self, n_copy, director)
			self.equal2_val{1}{2}(n_copy,:) = [nan*ones(numel(n_copy),1), zeros(numel(n_copy),3), ones(numel(n_copy),1).*director, zeros(numel(n_copy),1), ones(numel(n_copy),1)];
            self.pipe.equal2{1}{2}(n_copy,:) = [nan*ones(numel(n_copy),1), zeros(numel(n_copy),3), ones(numel(n_copy),1).*director, zeros(numel(n_copy),1), ones(numel(n_copy),1)];
            self.i_S = self.i_S + 1;
        end

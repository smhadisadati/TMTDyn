...
if par.dyn ~= 0
    ...
    sim_steps = par.t_init : par.dt_anim_rep(1) : par.t_final - par.dt_anim_rep(1) ;
    for t0_step = sim_steps
        % step sim:
        par.t0 = t0_step ;
        ...
        if sim_steps > 1
            post_proc( t(1:end-1) , t_equil_all , z(1:end-1,:) , z_equil_all , rjtip(:,:,1:end-1) , rjtip_equil_all , Qjtip(:,:,1:end-1) , Qjtip_equil_all , ...
                Phi_modal , M_modal , K_modal , V_modal , omega , xi , ...
                par ) ;
        end
